...
export const StudentView: React.FC<StudentViewProps> = ({ student, activity, existingSubmission, onSuccessSubmission, onBack }) => {
  ...
  const handleSheetClick = (e: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
    ...
    const xPct = Number((((clientX - rect.left) / rect.width) * 100).toFixed(2));
    const yPct = Number((((clientY - rect.top) / rect.height) * 100).toFixed(2));
                ...
                >
                    ...
                    <div className="bg-blue-100/95 text-blue-900 font-black text-xs px-2.5 py-1 rounded border-2 border-blue-600 shadow-md flex items-center gap-1 whitespace-nowrap">
                      ...
                      {!isReadOnly && (
                        <button
                          ...
                          className="p-1 hover:bg-rose-200 text-rose-700 rounded transition-colors"
                          title="حذف"
                        >
                  ...
                  {ann.type === 'check' && (
                    <div 
                      onClick={(e) => handleRemoveAnnotation(ann.id, e)}
                      onTouchEnd={(e) => handleRemoveAnnotation(ann.id, e)}
                      className="flex items-center justify-center bg-emerald-500 text-white border-2 border-white font-black rounded-full w-8 h-8 shadow-lg relative cursor-pointer active:scale-125 transition-transform"
                    >
                      ✓
                      {!isReadOnly && (
                        <button
                          ...
                          onClick={(e) => handleRemoveAnnotation(ann.id, e)}
                          onTouchEnd={(e) => handleRemoveAnnotation(ann.id, e)}
                          className="absolute -top-2 -right-2 bg-rose-600 text-white p-1 rounded-full shadow-md transition-opacity border border-white flex items-center justify-center"
                          title="حذف"
                        >
                          <X className="w-3 h-3" />
                        </button>
                  ...
                  {ann.type === 'cross' && (
                    <div 
                      onClick={(e) => handleRemoveAnnotation(ann.id, e)}
                      onTouchEnd={(e) => handleRemoveAnnotation(ann.id, e)}
                      className="flex items-center justify-center bg-rose-500 text-white border-2 border-white font-black rounded-full w-8 h-8 shadow-lg relative cursor-pointer active:scale-125 transition-transform"
                    >
                      ✕
                      {!isReadOnly && (
                        <button
